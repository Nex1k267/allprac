...
namespace Practika10
{
    abstract class Department
    {
        ...
        {
            doctors.Add(doctor);
            NotifyDoctors("В отделение " + name + " добавлен новый врач: " + doctor.FirstName + " " + doctor.LastName);
        }
        ...
        public bool RemoveDoctor(int doctorId)
        {
            for (int i = 0; i < doctors.Count; i++)
            {
                ...
                {
                    string fio = doctors[i].FirstName + " " + doctors[i].LastName;
                    doctors.RemoveAt(i);
        ...
        protected void NotifyDoctors(string message)
        {
            for (int i = 0; i < doctors.Count; i++)
            {
                Console.WriteLine("Уведомление для доктора " + doctors[i].FirstName + " " + doctors[i].LastName + " из отделения " + name + ": " + message);
            }
        }

        public void NotifyNewPatient(string patientFullName)
        {
            NotifyDoctors("Поступил новый пациент: " + patientFullName);
        }
